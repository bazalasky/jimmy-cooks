...
function getSearch(searchText) {
    ...
    .then((data) => {
        ...
        $.each (movies, (index, movie) => {
            output +=
            '<div class="col-md-3" style="margin-top: 20px; margin-bottom: 20px; cursor: pointer;" onclick="movieSelected(' + movie.id + ')"><div class="card text-center"><img src="' + baseImageURL + movie.poster_path + '"><h5>' + movie.title +'</h5><p>' + movie.release_date + '</p><a onclick="movieSelected(' + movie.id + ')" class="btn btn-primary" href="#">Movie Details</a></div></div>';
...
function getNowPlaying() {
    ...
    .then((data) => {
        ...
        let output = '';
        $.each(movies, (index, movie) => {
            output += '<div class="col-md-3" style="margin-top: 20px; margin-bottom: 20px; cursor: pointer;" onclick="movieSelected(' + movie.id + ')"><div class="card text-center"><img src="' + baseImageURL + movie.poster_path + '"><h5>' + movie.title +'</h5><p>' + movie.release_date + '</p><a onclick="movieSelected(' + movie.id + ')" class="btn btn-primary" href="#">Movie Details</a></div></div>';
